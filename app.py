...
def cria_trecho():
    trecho = {}
    trecho['id'] = input('id: ')
    trecho['comprimento [m]'] = float(input('Comprimento [m]: '))
    trecho['curvas'] = float(input('numero de curvas: '))
    trecho['angulo total [°]'] = trecho['curvas'] * 90
    trecho['deriva de'] = input('deriva de: ')
    trecho['vazao [m³/h]'] = 0
    trecho['diametro [mm]'] = 0
    trecho['velocidade [m/s]'] = 0.0
    trecho['difusor [m³/h]'] = 0
    trecho['difusor [Pa]'] = 0
    return trecho


def incluir_difusor(trechos):
    escolha = input('trecho: ')
    for i in trechos:
        if i['id'] == escolha:
            i['difusor [m³/h]'] = float(input('vazão difusor [m³/h]: '))
            i['difusor [Pa]'] = float(input('perda difusor [Pa]: '))
